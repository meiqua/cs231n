...
def batchnorm_backward_alt(dout, cache):
  ...
  x = cache['x']
  var = cache['sample_var']+cache['eps']
  mean = cache['sample_mean']
  dx = cache['gamma']/np.sqrt(var)*(dout-np.sum(dout,axis=0)/m-(x-mean)/var*np.sum(dout*(x-mean),axis=0)/m)
